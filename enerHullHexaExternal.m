function e = enerHullHexaExternal(coor,cutoff,hull,centers)
...
% hull - The hexagonal hull.
% centers - The (x,y) locations of the detected particles.
% The output is:
% e - The computed energy.

coorHull = convertToHexaExternal(hull,coor(1:2),coor(3));
shift=-cutoff^2;
centXRep=repmat(centers(:,1),1,6);
hXReshape=reshape(coorHull(:,1),1,[]);
centYRep=repmat(centers(:,2),1,6);
