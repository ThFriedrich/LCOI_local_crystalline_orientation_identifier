function tests = testKeepUniqueMinTri
...
function testEmptyMasks(testCase)
    ...
    app.coorOfMasksRect = [];
    app.hullHexa = [1]; % Some arbitrary hull parameter
    
    % Call the function
    indReturn = app.keepUniqueMinTri();
...
function testSingleMask(testCase)
    ...
    app.coorOfMasksTri = [10, 5, 0]; % Single mask 
    app.hullTri = [1];
    ...
    testCase.verifyEqual(indReturn, ones(1, size(app.coorOfMasksTri, 1)));
end
...
function testDistantMasks(testCase)
    ...
    app.hullTri = [1]; 
    cutoff = app.hullTri(1) /(sqrt(3)*2); % 1//(sqrt(3)*2)
...
function testClusteredMasks(testCase)
    ...
    app.coorOfMasksTri = [
        0, 0, 0;      % Best mask overall at (0,0)
        0, 0.1, 0;      % Mask at (0, 0.1) - distance = 0.1 < cutoff from (0,0)
        5, 0, 0;      % Best mask of second cluster at (5,0) 
        5, 0.1, 0;  % Mask at (5,0.1) - distance = 0.1 < cutoff from (5,0)
    ];
...
function testComplexClusteredMasks(testCase)
    ...
    app.coorOfMasksTri = [
        0, 0, 0;      % Best mask overall at (0,0)
        0.2 ,0, 0;      % Distance = 0.2 < cutoff from (0,0)
        0.3, 0.0, 0;    % Far enough from 1 but not from 2 (which is not kept) 
    ];
