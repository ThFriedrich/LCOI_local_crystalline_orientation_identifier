classdef LCOI_CrystalAnalysis_exported < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        indicesTopLeft % Indices of top-left pixel in the user-marked domain for zoom-in.
        indicesBottomRight % Indices of bottom-right pixel in the user-marked domain for zoom-in.
        ...
        indicesTopLeft2 % Indices of top-left pixel in the user-marked domain for contrast adjustment.
        indicesBottomRight2 % Indices of bottom-right pixel in the user-marked domain for contrast adjustment.
    end
    ...
    methods (Access = public)
        ...
        function [indReturn] = keepUniqueMinHexa(app)
            ...
            elseif size(app.coorOfMasksHexa,1)==1
                indReturn=[1];
            end
        end
        ...
        function  plotDirectionColor(app,ax,typeP)
            ...
            if typeP==1 % Hexagonal
                if isempty(app.coorOfMasksHexa)==false
                    ...
                    if v=='2'
                        ...
                        for i=1:size(app.coorOfMasksHexa,1)
            ...
            elseif typeP==3 % Triangular
                if isempty(app.coorOfMasksTri)==false
                    ...
                    for i=1:size(app.coorOfMasksTri,1)
                        if isnan(col(i,:))==false
        ...
        function [indReturn] = keepUniqueMinRect(app)
            ...
            elseif size(app.coorOfMasksRect,1)==1
                indReturn=[1];
            end
        end
        ...
        function [indReturn] = keepUniqueMinTri(app)
            ...
            elseif size(app.coorOfMasksTri,1)==1
                indReturn=[1];
            end
        end
        ...
        function identifyRect(app)
            ...
            app.eOfMasksRect=[];
            app.indPerCentRect=[];
            app.indPerCentRectNotSorted=[];
            app.coorOfMasksRectNotSorted=[];
            app.eofNotSortedMasksRect=[];
            if ~isempty(app.imageP) && ~isempty(app.centers)
                cla(app.UIAxes2)
        ...
        % inspired by: https://de.mathworks.com/matlabcentral/answers/419823-how-to-catch-warnings
        % Infos on toolbox names for the ver()-command: https://de.mathworks.com/matlabcentral/answers/377731-how-do-features-from-license-correspond-to-names-from-ver#answer_430148
        function b_check = checkForToolbox(~,toolbox)
            % Check if a toolbox is available and return a flag accordingly
            ...
            % call ver, which return the version of the toolbox or a warning when not installed
            ver(toolbox)

            % see if a warning was raised
            if(isempty(lastwarn()))
                b_check = 1;
            else
                b_check = 0;
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            % Check for the required toolboxes: Image Processing toolbox and the Parallel Computation toolbox
            if checkForToolbox(app,'images')==0
                errordlg('You do not have the image processing toolbox. It is required for the functionality of the software.','Missing toolbox');
            end
            if checkForToolbox(app,'parallel')==0
                errordlg('You do not have the parallel computation toolbox. Serial computation will be used instead.','Missing toolbox');
            end
        end
        ...
        function PickparticlesformaskhexagonButtonPushed(app, event)
            ...
            if isempty(app.centers)==false
                ...
                % First point
                point_valid = false;
                max_tries = 3; % Cap on number of attempts
                try_count = 0;

                while ~point_valid && try_count < max_tries
                    ...
                    % Validate point is inside axis bounds
                    if x1 >= app.UIAxes2.XLim(1) && x1 <= app.UIAxes2.XLim(2) && ...
                            y1 >= app.UIAxes2.YLim(1) && y1 <= app.UIAxes2.YLim(2)
                        point_valid = true;
                    else
                        app.InstructionsTextArea.Value=sprintf('Point outside axis. Please click inside the axis. (%d/%d tries)', try_count, max_tries);
                        % Give user time to read message
                        pause(5);
                        if try_count < max_tries
                            app.InstructionsTextArea.Value='Please click as close to a "tip" particle as you can assuming that the long axis is vertical (in case of 2-fold symmetry).';
                        else
                            app.InstructionsTextArea.Value='Maximum attempts reached. Selection canceled.';
                ...
                for i=1:5
                    ...
                    % Add validation loop for each point
                    point_valid = false;
                    try_count = 0;
        ...
        function IdentifyorientationshexagonButtonPushed(app, event)
            ...
            if isempty(app.hullHexa)==false
                ...
                if v==['Off']
                    identifyHexa(app)
                elseif v==['On'] && checkForToolbox(app,'parallel')==1
                    identifyHexaParallel(app)
                else
                    errordlg('You do not have the parallel computation toolbox. Serial computation is used.','Missing toolbox');
                    app.ParallelcomputationSwitch.Value=['Off'];
                    app.ParallelcomputationSwitch_2.Value=['Off'];
                    app.ParallelcomputationSwitch_3.Value=['Off'];
                    identifyHexa(app)
                end
            else
        ...
        function ChooseparticleButtonPushed(app, event)
            ...
            if isempty(app.centers)==false && ~isempty(app.imageP)
                point_valid = false;
                max_tries = 3; % Cap on number of attempts
                try_count = 0;
                while ~point_valid && try_count < max_tries
                    ...
                    app.UIFigure.HandleVisibility = fhv;
                    % Validate point is inside axis bounds
                    if indices1(1) >= app.UIAxes2_2.XLim(1) && indices1(1) <= app.UIAxes2_2.XLim(2) && ...
                            indices1(2) >= app.UIAxes2_2.YLim(1) && indices1(2) <= app.UIAxes2_2.YLim(2)
                        point_valid = true;
                    else
                        errordlg(sprintf('Point outside axis. Please click inside the axis. (%d/%d tries)', try_count, max_tries),'Picking Error!');
                        pause(3);
                        if try_count == max_tries
                            errordlg('Maximum attempts reached. Selection canceled.','Picking Error!');
                            return; % Exit the callback function
                        end
                    end
                end
                dist=sqrt((app.centers(:,1)-indices1(1)).^2+(app.centers(:,2)-indices1(2)).^2);
                [~,minInd]=min(dist);
        ...
        function MarkDomainButtonPushed(app, event)
            ...
            if ~isempty(app.imageP)
                ...
                app.indicesTopLeft=indices1;
                scatter(app.UIAxes2_2,indices1(1),indices1(2),'.red')
                ...
                app.indicesBottomRight=indices1;
                scatter(app.UIAxes2_2,indices1(1),indices1(2),'.red')
                ...
                app.indicesTopLeft2=[xMin,yMin];
                app.indicesBottomRight2=[xMax,yMax];
        ...
        function ImAdjustindomainButtonPushed(app, event)
            ...
            if isempty(app.imagePTemp)
                app.imagePTemp=app.imageP;
            end
            if ~isempty(app.indicesTopLeft2)
                xMin=app.indicesTopLeft2(1);
                yMin=app.indicesTopLeft2(2);
                xMax=app.indicesBottomRight2(1);
                yMax=app.indicesBottomRight2(2);
                ...
                errordlg('Pick domain first.','Picking Error!')
            end
        end
        ...
        function RemovesegmentedButtonPushed(app, event)
            ...
            if ~isempty(app.imageP) && ~isempty(app.domain)
                app.ReadyLamp.Color='red';
                app.ReadyLabel.Text='Busy';
                drawnow()
        ...
        function AddpointtoregionButtonPushed(app, event)
            ...
            if ~isempty(app.imageP)
                ...
                while ~point_valid && try_count < max_tries
                    ...
                    app.UIFigure.HandleVisibility = fhv;
                    % Validate point is inside axis bounds
                    if indices1(1) >= app.UIAxes2_3.XLim(1) && indices1(1) <= app.UIAxes2_3.XLim(2) && ...
                            indices1(2) >= app.UIAxes2_3.YLim(1) && indices1(2) <= app.UIAxes2_3.YLim(2)
                        point_valid = true;
                    else
                        errordlg(sprintf('Point outside axis. Please click inside the axis. (%d/%d tries)', try_count, max_tries),'Picking Error!');
                        pause(3);
                ...
                scatter(app.UIAxes2_3,indices(1),indices(2),'.red')
                app.region(end+1,1:2)=indices;
            end
        end
        ...
        function OptimizemaskhexagonButtonPushed(app, event)
            ...
            if isempty(app.hullHexa)==false
                ...
                elseif v==['On'] && checkForToolbox(app,'parallel')==1
                    ...
                    errordlg('You do not have the parallel computation toolbox. Serial computation is used.','Missing toolbox');                    app.ParallelcomputationSwitch.Value=['Off'];
                    app.ParallelcomputationSwitch_2.Value=['Off'];
                    app.ParallelcomputationSwitch_3.Value=['Off'];
                    identifyHexa(app)
                end
        ...
        function PickdomainButtonPushed(app, event)
            ...
            if ~isempty(app.imageP)
                ...
                while ~point_valid && try_count < max_tries
                    ...
                    app.UIFigure.HandleVisibility = fhv;
                    % Validate point is inside axis bounds
                    if indices1(1) >= app.UIAxes2.XLim(1) && indices1(1) <= app.UIAxes2.XLim(2) && ...
                            indices1(2) >= app.UIAxes2.YLim(1) && indices1(2) <= app.UIAxes2.YLim(2)
                        point_valid = true;
                    else
                        errordlg(sprintf('Point outside axis. Please click inside the axis. (%d/%d tries)', try_count, max_tries),'Picking Error!');
                        pause(3);
                ...
                app.indicesTopLeft=indices1;
                scatter(app.UIAxes2,indices1(1),indices1(2),'.blue')
        ...
        function AssigndirectionsclusterwithminimalenergyButtonPushed(app, event)
            ...
            if ~isempty(app.imageP) && ~isempty(app.coorOfMasksHexa)
                app.ReadyLamp.Color='red';
                app.ReadyLabel.Text='Busy';
                drawnow()
                app.directions={};
        ...
        function FlipallButtonPushed(app, event)
            ...
            if ~isempty(app.imageP) && ~isempty(app.directions)
                app.ReadyLamp.Color='red';
                app.ReadyLabel.Text='Busy';
                drawnow()
        ...
        function PickparticlesformaskrectangleButtonPushed(app, event)
            % Initiating rectangular hull formation by allowing the user to choose 4 points using the mouse.
            hold(app.UIAxes2,'on')
            if isempty(app.centers)==false
                hullT=NaN(4,2);
        ...
        function PlotdeviationsfrommeansButtonPushed(app, event)
            ...
            % hulls.
            if ~isempty(app.imageP) && size(app.indPerCentRect,1)>0
                app.ReadyLamp.Color='red';
                app.ReadyLabel.Text='Busy';
                drawnow()
                cla(app.UIAxes2_5)
        ...
        function PickpointsinmiddleimageButtonPushed(app, event)
            ...
            while ~point_valid && try_count < max_tries
                ...
                app.UIFigure.HandleVisibility = fhv;
                % Validate point is inside axis bounds
                if indices1(1) >= app.UIAxes_2.XLim(1) && indices1(1) <= app.UIAxes_2.XLim(2) && ...
                        indices1(2) >= app.UIAxes_2.YLim(1) && indices1(2) <= app.UIAxes_2.YLim(2)
                    point_valid = true;
                else
                    errordlg(sprintf('Point outside axis. Please click inside the axis. (%d/%d tries)', try_count, max_tries),'Picking Error!');
                    pause(3);
            ...
            while ~point_valid && try_count < max_tries
                ...
                app.UIFigure.HandleVisibility = fhv;
                % Validate point is inside axis bounds
                if indices2(1) >= app.UIAxes_2.XLim(1) && indices2(1) <= app.UIAxes_2.XLim(2) && ...
                        indices2(2) >= app.UIAxes_2.YLim(1) && indices2(2) <= app.UIAxes_2.YLim(2)                    point_valid = true;
                else
                    errordlg(sprintf('Point outside axis. Please click inside the axis. (%d/%d tries)', try_count, max_tries),'Picking Error!');
                    pause(3);
                    if try_count == max_tries
            ...
            plot(app.UIAxes_2,[indices1(1),indices2(1)],[indices1(2),indices2(2)],'LineStyle','--','LineWidth',2)
        ...
        function CentersoptimizationButtonPushed(app, event)
            ...
            if ~isempty(app.imageP)
                ...
                if isempty(app.radii)==false
                    ...
                    if  checkForToolbox(app,'parallel')==1
                        parpool('local', maxNumCompThreads);
                        cent=app.centers;
        ...
        function PlotdeviationsButtonPushed(app, event)
            ...
            % hulls.
            if ~isempty(app.imageP) && size(app.indPerCentHexa,1)>0
                app.ReadyLamp.Color='red';
                app.ReadyLabel.Text='Busy';
                drawnow()
                cla(app.UIAxes2_4)
        ...
        function PickparticlesformasktriangularButtonPushed(app, event)
            % Initiating triangular hull formation by allowing the user to choose 3 points using the mouse.
            hold(app.UIAxes2,'on')
            if isempty(app.centers)==false
                hullT=NaN(3,2);
                app.InstructionsTextArea.Value='Please click as close to the desired particle as you can.';
        ...
        function ClearButtonPushed(app, event)
            ...
            axis(app.UIAxes_2,"image")
            hold(app.UIAxes_2,'on')
            app.DistanceEditField.Value=0.0;
        end
        ...
        function PlotdeviationsfrommeansButton_3Pushed(app, event)
            ...
            if ~isempty(app.imageP) && size(app.indPerCentTri,1)>0
                app.ReadyLamp.Color='red';
                app.ReadyLabel.Text='Busy';
                drawnow()
                cla(app.UIAxes2_5)
        ...
        function OrdermasksfortessellationButtonPushed(app, event)
            ...
            if  checkForToolbox(app,'parallel')==1
                ...
                cent=app.centers;
                coor=app.coorOfMasksHexa;
                indPerCentTemp=NaN(size(coor,1),6);
                for i=1:size(coor,1)
                    minList=NaN(1,6);
                    centHu=coor(i,1:2);
                    ang=coor(i,3);
        ...
        function PlotButton_4Pushed(app, event)
            ...
            if isempty(coor)==false && ~isempty(app.imageP)
                Cmap=zeros(256,4);
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = LCOI_CrystalAnalysis_exported
